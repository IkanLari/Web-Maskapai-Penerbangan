...
// Query untuk mengambil data tiket yang statusnya progress dan maskapai sesuai dengan user yang login
$query = "SELECT t.*, t.total_harga as harga_tiket, p.maskapai, p.asal, p.tujuan, p.tanggal, p.waktu, u.username as nama_user
          ...
          WHERE t.status = 'progress' AND p.maskapai = '".$_SESSION['username']."'
...
<head>
    ...
    <title>Konfirmasi Customer</title>
    <style>
        /* General styles */
        body {
            margin: 0;
...
<body>
    <header>
        ...
        <nav>
            ...
            <a href="konfirmasiCustomer.php" style="background-color: rgba(255, 255, 255, 0.4);">Konfirmasi Customer</a>
            <a href="history.php">History</a>
            <a href="dashboardMaskapai.php">Dashboard</a>
        </nav>
    </header>
    <main>
        <h2>Konfirmasi Customer</h2>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="action-links">
                        <a href="javascript:void(0);" class="accept" onclick="confirmCustomer(<?php echo $row['id_tiket']; ?>, 'accepted')">Accept</a>
                        <a href="javascript:void(0);" class="reject" onclick="confirmCustomer(<?php echo $row['id_tiket']; ?>, 'rejected')">Reject</a>
                    </td>
                </tr>
                <?php 
                }
    ...
    <script>
        function confirmCustomer(id_tiket, status) {
            const action = status === 'accepted' ? 'menerima' : 'menolak';
            if (confirm(`Apakah Anda yakin ingin ${action} tiket ini?`)) {
                fetch("./proses/maskapai_proses_konfirmasi_customer.php?id_tiket=" + id_tiket + "&status=" + status, {
                    method: 'GET'
                })
                .then(response => response.text())
                .then(data => {
                    alert(`Status tiket diubah menjadi "${status}"`);
                    location.reload();
                })
                .catch(error =>
